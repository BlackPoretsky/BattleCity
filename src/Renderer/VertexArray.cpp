...
namespace RenderEngine {
	...
	void VertexArray::addBuffer(const VertexBuffer& vertexBuffer, const VertexBufferLayout& layout)
	{
		...
		for (unsigned int i = 0; i < layoutElements.size(); ++i)
		{
			...
			GLuint currenAttributeIndex = m_elementsCount + i;
			glEnableVertexAttribArray(currenAttributeIndex);
			glVertexAttribPointer(currenAttributeIndex, currentLayoutElement.count, currentLayoutElement.type, currentLayoutElement.normalized, layout.getStride(), offset);
